...
## 0. 2026-03 自适应布局定稿（本次实现基线）

本节是本次代码改造的唯一实现基线，优先级高于后文历史草案。

### 0.1 目标

- 布局按当前窗格数量自动适配，不再依赖“同屏上限”滑杆来决定布局。
- 4窗格在 `1主多副` 模式下固定为“左侧1主 + 右侧3个纵向副屏”。
- 支持 1-16 窗格的可解释规则，保证每一种数量都有明确行为。

### 0.2 三种布局模式的最终规则

补充：同屏最大数量不再由用户手动设置，改为根据当前窗口尺寸自动识别。

当前默认策略：性能优先。

- 桌面端：更大单窗格阈值（约 420x260 逻辑像素），自动上限硬顶 12。
- 移动端：更大单窗格阈值（约 280x190 逻辑像素），自动上限硬顶 6。
...
| 总窗格数 | 等分布局 | 1主多副（右侧副屏） | 2主多副（右侧副屏） |
|---|---|---|---|
| 1 | 1列x1行 | - | - |
| 2 | 2列x1行 | 副屏: 1列x1行 | 主屏: 2个，副屏: 无 |
| 3 | 2列x2行 | 副屏: 1列x2行 | 副屏: 1列x1行 |
| 4 | 2列x2行 | 副屏: 1列x3行（关键） | 副屏: 2列x1行 |
| 5 | 3列x2行 | 副屏: 2列x2行 | 副屏: 2列x2行 |
| 6 | 3列x2行 | 副屏: 2列x3行 | 副屏: 2列x2行 |
| 7 | 3列x3行 | 副屏: 2列x3行 | 副屏: 3列x2行 |
| 8 | 3列x3行 | 副屏: 3列x3行 | 副屏: 3列x2行 |
| 9 | 3列x3行 | 副屏: 3列x3行 | 副屏: 3列x3行 |
| 10 | 3列x4行 | 副屏: 3列x3行 | 副屏: 3列x3行 |
| 11 | 3列x4行 | 副屏: 3列x4行 | 副屏: 3列x3行 |
| 12 | 3列x4行 | 副屏: 4列x3行 | 副屏: 4列x3行 |
| 13 | 4列x4行 | 副屏: 4列x3行 | 副屏: 4列x3行 |
| 14 | 4列x4行 | 副屏: 4列x4行 | 副屏: 4列x3行 |
| 15 | 4列x4行 | 副屏: 4列x4行 | 副屏: 4列x4行 |
| 16 | 4列x4行 | 副屏: 4列x4行 | 副屏: 4列x4行 |

### 0.4 复查记录（编码前）

- 第1轮复查：确认 4窗格 `1主多副` 必定为 1列x3行，不再出现横向三格。
- 第2轮复查：确认 1-16 每个数量在三种布局下都有确定结果。
- 第3轮复查：确认规则与实现可一一映射（代码中用统一函数返回列/行）。

### 0.5 实现约束

- 只改布局计算与布局面板文案，不动播放器生命周期。
- 保留同屏上限能力（防资源过载），但不再放在布局面板里误导用户。
- 布局面板强调“数量自适应”，主屏比例只在主副模式下可调。

...
class SettingsService extends GetxController {
  ...
  final enableDynamicTheme =
      (HivePrefUtil.getBool('enableDynamicTheme') ?? false).obs;
  final themeColorSwitch =
      (HivePrefUtil.getString('themeColorSwitch') ?? Colors.blue.hex).obs;
  ...
  final enableDenseFavorites =
      (HivePrefUtil.getBool('enableDenseFavorites') ?? true).obs;
  final enableBackgroundPlay =
      (HivePrefUtil.getBool('enableBackgroundPlay') ?? false).obs;
  final enableStartUp = (HivePrefUtil.getBool('enableStartUp') ?? true).obs;
  final enableRotateScreenWithSystem =
      (HivePrefUtil.getBool('enableRotateScreenWithSystem') ?? false).obs;
  final enableScreenKeepOn =
      (HivePrefUtil.getBool('enableScreenKeepOn') ?? true).obs;
  final enableAutoCheckUpdate =
      (HivePrefUtil.getBool('enableAutoCheckUpdate') ?? true).obs;
  final enableFullScreenDefault =
      (HivePrefUtil.getBool('enableFullScreenDefault') ?? false).obs;
  ...
  final enableAutoShutDownTime =
      (HivePrefUtil.getBool('enableAutoShutDownTime') ?? false).obs;
  ...
  final danmakuBottomArea =
      (HivePrefUtil.getDouble('danmakuBottomArea') ?? 0.5).obs;
  final danmakuSpeed = (HivePrefUtil.getDouble('danmakuSpeed') ?? 8.0).obs;
  final danmakuFontSize =
      (HivePrefUtil.getDouble('danmakuFontSize') ?? 16.0).obs;
  final danmakuFontBorder =
      (HivePrefUtil.getDouble('danmakuFontBorder') ?? 4.0).obs;
  ...
  final playerCompatMode =
      (HivePrefUtil.getBool('playerCompatMode') ?? false).obs;
  final customPlayerOutput =
      (HivePrefUtil.getBool('customPlayerOutput') ?? false).obs;
  final videoOutputDriver =
      (HivePrefUtil.getString('videoOutputDriver') ?? "gpu").obs;
  final audioOutputDriver =
      (HivePrefUtil.getString('audioOutputDriver') ?? "auto").obs;
  final videoHardwareDecoder =
      (HivePrefUtil.getString('videoHardwareDecoder') ?? "auto").obs;
  ...
  final hotAreasList =
      ((HivePrefUtil.getStringList('hotAreasList') ?? AppConsts.supportSites))
          .obs;
  final preferResolution =
      (HivePrefUtil.getString('preferResolution') ??
              PlayerConsts.resolutions[0])
          .obs;
  final preferPlatform =
      (HivePrefUtil.getString('preferPlatform') ?? AppConsts.platforms[0]).obs;
  final multiLiveQueueConsumeMode =
      (HivePrefUtil.getString('multiLiveQueueConsumeMode') ?? 'keep').obs;
  final multiLiveMaxTiles = (HivePrefUtil.getInt('multiLiveMaxTiles') ?? 9).obs;
  final multiLiveMainRatio =
      (HivePrefUtil.getDouble('multiLiveMainRatio') ?? 0.5).obs;
  final multiLiveAudioMode =
      (HivePrefUtil.getString('multiLiveAudioMode') ?? 'focus').obs;
  final multiLiveGlobalVolume =
      (HivePrefUtil.getDouble('multiLiveGlobalVolume') ?? 1.0).obs;

  // ==============================
  // ❤️ 收藏 & 历史
  // ==============================
  final favoriteRooms =
      ((HivePrefUtil.getStringList('favoriteRooms') ?? [])
              .map((e) => LiveRoom.fromJson(jsonDecode(e)))
              .toList())
          .obs;
  final historyRooms =
      ((HivePrefUtil.getStringList('historyRooms') ?? [])
              .map((e) => LiveRoom.fromJson(jsonDecode(e)))
              .toList())
          .obs;
  final favoriteAreas =
      ((HivePrefUtil.getStringList('favoriteAreas') ?? [])
  ...
  final currentWebDavConfig =
      (HivePrefUtil.getString('currentWebDavConfig') ?? '').obs;
  final webDavConfigs =
      ((HivePrefUtil.getStringList('webDavConfigs') ?? [])
              .map((e) => WebDAVConfig.fromJson(jsonDecode(e)))
              .toList())
          .obs;
  final m3uDirectory =
      (HivePrefUtil.getString('m3uDirectory') ?? 'm3uDirectory').obs;

  // ==============================
  // 🎨 颜色映射（静态）
  // ==============================
  final Map<ColorSwatch<Object>, String> colorsNameMap = AppConsts.themeColors
      .map(
  ...
  void onInit() {
    ...
    debounce(enableAutoShutDownTime, (callback) {
      HivePrefUtil.setBool(
        'enableAutoShutDownTime',
        enableAutoShutDownTime.value,
      );
    ...
    multiLiveQueueConsumeMode.listen((value) {
      HivePrefUtil.setString('multiLiveQueueConsumeMode', value);
    });

    multiLiveMaxTiles.listen((value) {
      HivePrefUtil.setInt('multiLiveMaxTiles', value.clamp(1, 16));
    });

    multiLiveMainRatio.listen((value) {
      HivePrefUtil.setDouble('multiLiveMainRatio', value.clamp(0.3, 0.7));
    });

    multiLiveAudioMode.listen((value) {
      HivePrefUtil.setString('multiLiveAudioMode', value);
    });

    multiLiveGlobalVolume.listen((value) {
      HivePrefUtil.setDouble('multiLiveGlobalVolume', value.clamp(0.0, 1.0));
    });
    ...
    webDavConfigs.listen((configs) {
      HivePrefUtil.setStringList(
        'webDavConfigs',
        configs.map<String>((e) => jsonEncode(e.toJson())).toList(),
      );
    });
  ...
  LiveRoom getLiveRoomByRoomId(String roomId, String platform) {
    ...
    return favoriteRooms.firstWhere(
      ...
      orElse: () => historyRooms.firstWhere(
        (element) => element.roomId == roomId && element.platform == platform,
      ),
    );
  }
  ...
  bool isFavoriteArea(LiveArea area) {
    return favoriteAreas.any(
      (element) =>
          element.areaId == area.areaId &&
          element.platform == area.platform &&
          element.areaType == area.areaType,
    );
  }
  ...
  bool updateWebDavConfig(WebDAVConfig config) {
    int idx = webDavConfigs.indexWhere(
  ...
  void fromJson(Map<String, dynamic> json) {
    ...
    favoriteRooms.value = safeParseList<LiveRoom>(
      json['favoriteRooms'],
      (m) => LiveRoom.fromJson(m),
    );
    favoriteAreas.value = safeParseList<LiveArea>(
      json['favoriteAreas'],
      (m) => LiveArea.fromJson(m),
    );
    shieldList.value = json['shieldList'] != null
    ...
    themeModeName.value = AppConsts.themeModes.keys.firstWhere(
      (e) => e == json['themeMode'],
      orElse: () => "System",
    );
    ...
    enableRotateScreenWithSystem.value =
        json['enableRotateScreenWithSystem'] ?? false;
    ...
    languageName.value = AppConsts.languages.keys.firstWhere(
      (e) => e == json['languageName'],
      orElse: () => "简体中文",
    );
    ...
    multiLiveQueueConsumeMode.value =
        json['multiLiveQueueConsumeMode'] == 'removeAdded' ||
            json['multiLiveQueueConsumeMode'] == 'clearAll'
        ? json['multiLiveQueueConsumeMode']
        : 'keep';
    multiLiveMaxTiles.value =
        (json['multiLiveMaxTiles'] is int
                ? json['multiLiveMaxTiles'] as int
                : 9)
            .clamp(1, 16);
    multiLiveMainRatio.value =
        (json['multiLiveMainRatio'] is num
                ? (json['multiLiveMainRatio'] as num).toDouble()
                : 0.5)
            .clamp(0.3, 0.7);
    multiLiveAudioMode.value =
        json['multiLiveAudioMode'] == 'mix' ||
            json['multiLiveAudioMode'] == 'mainPriority'
        ? json['multiLiveAudioMode']
        : 'focus';
    multiLiveGlobalVolume.value =
        (json['multiLiveGlobalVolume'] is num
                ? (json['multiLiveGlobalVolume'] as num).toDouble()
                : 1.0)
            .clamp(0.0, 1.0);
    ...
    danmakuArea.value = json['danmakuArea'] != null
        ...
        : 1.0;
    danmakuBottomArea.value = double.parse(
      json['danmakuBottomArea'].toString(),
    );
    danmakuSpeed.value = json['danmakuSpeed'] != null
        ? double.parse(json['danmakuSpeed'].toString())
        : 8.0;
    danmakuFontSize.value = json['danmakuFontSize'] != null
        ? double.parse(json['danmakuFontSize'].toString())
        : 16.0;
    danmakuFontBorder.value = json['danmakuFontBorder'] != null
        ? double.parse(json['danmakuFontBorder'].toString())
        : 4.0;
    danmakuOpacity.value = json['danmakuOpacity'] != null
        ? double.parse(json['danmakuOpacity'].toString())
        : 1.0;
    ...
    videoOutputDriver.value =
        PlayerConsts.videoOutputDrivers.keys.contains(json['videoOutputDriver'])
        ? json['videoOutputDriver']
        : 'gpu';
    audioOutputDriver.value =
        PlayerConsts.audioOutputDrivers.keys.contains(json['audioOutputDriver'])
        ? json['audioOutputDriver']
        : 'auto';
    videoHardwareDecoder.value =
        PlayerConsts.hardwareDecoder.keys.contains(json['videoHardwareDecoder'])
        ? json['audioOutputDriver']
        : 'auto';
  ...
  Map<String, dynamic> toJson() {
    ...
    json['favoriteRooms'] = favoriteRooms
        .map<String>((e) => jsonEncode(e.toJson()))
        .toList();
    json['webDavConfigs'] = webDavConfigs
        .map<String>((e) => jsonEncode(e.toJson()))
        .toList();
    json['favoriteAreas'] = favoriteAreas
        .map<String>((e) => jsonEncode(e.toJson()))
        .toList();
    ...
    json['multiLiveQueueConsumeMode'] = multiLiveQueueConsumeMode.value;
    json['multiLiveMaxTiles'] = multiLiveMaxTiles.value;
    json['multiLiveMainRatio'] = multiLiveMainRatio.value;
    json['multiLiveAudioMode'] = multiLiveAudioMode.value;
    json['multiLiveGlobalVolume'] = multiLiveGlobalVolume.value;
    ...
    json['hotAreasList'] = hotAreasList
